...
@export var crop_id := "turnip"
@export var seed_item_id := "seed_turnip"
@export var days_to_mature := 4
...
func _update_visuals() -> void:
	...
	if planted:
		var progress := clamp(float(growth_days + 1) / float(max(days_to_mature, 1)), 0.2, 1.0)
